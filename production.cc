...
// solape spanish ==> english overlap
float ProductionB::solape(Hypothesis *a, Hypothesis *b) {
	...
	if (s >= x && t >= y) {
		...
		float aTotal = (a->parent->s - a->parent->x + 1.0)
				* (a->parent->t - a->parent->y + 1.0);
...
bool ProductionB::check_out() {
	if (check_str(outStr, (char*) "$1") < 0
			&& check_str(outStr, (char*) "$2") < 0)
		return false;
	return true;
}
...
void ProductionH::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
	this->print();
	if (!H->hi->pt && H->hi->prod->tipo() == 'P' && !H->hi->hi->pt
			&& !H->hi->hi->hi->prod
...
void ProductionV::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
	...
	if (hdhiclass
			&& (!strcmp(hdhiclass, "\\sum") || !strcmp(hdhiclass, "\\int")
					|| !strcmp(hdhiclass, "-"))) {
...
void ProductionVe::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
...
void ProductionSSE::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
	this->print();
...
void ProductionSup::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
...
void ProductionSub::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
	this->print();
	fprintf(fout, "<msub>\n");
...
void ProductionIns::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
	this->print();
	*nid = *nid + 1;
...
void ProductionMrt::print_mathml(Grammar *G, Hypothesis *H, FILE *fout,
		int *nid) {
	this->print();
	*nid = *nid + 1;
...
/**
 * 0.01269354 OpBin dbar /
 * pr = 0.01269354, noTerminales['OpBin']=S, texStr = '/', clase = 'dbar', latex is '/'
 */
